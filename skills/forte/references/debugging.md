...
page shows a "Paused" badge). Resume the service from the console service page, or trigger a
deployment from the console — manually triggered deploys auto-resume a paused service, but GitHub
push/release builds are skipped (immediately cancelled) while it's paused — then re-test.
